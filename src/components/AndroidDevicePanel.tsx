      ...
      {!ready ? (
        <div className="rounded-xl border border-warning/25 bg-warning/10 p-4 text-[12px] leading-relaxed text-warning">
          {selected.state === "unauthorized"
            ? t("misc.androidDevice.unauthorized")
            : t("misc.androidDevice.deviceState", { state: selected.state })}
        </div>
      ) : (
        <>
          <div
            role="application"
            aria-label={t("misc.androidDevice.screenAria", { name: deviceLabel(selected) })}
            tabIndex={0}
            onKeyDown={keyDown}
            onWheel={wheel}
            onPointerDown={(event) => {
          ...
          >
            {frame ? (
              <img
                ref={imageRef}
                src={frame}
                alt={t("misc.androidDevice.screenAlt", { name: deviceLabel(selected) })}
                draggable={false}
                onLoad={(event) => {
                  setDimensions({
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            >
              <ArrowLeft size={13} /> {t("misc.androidDevice.back")}
            </button>
            <button
              onClick={() => send({ type: "key", key: "home" })}
              className="flex items-center justify-center gap-1.5 rounded-lg bg-raised py-2 text-[12px] text-ink hover:bg-raised-hover"
            >
              <Circle size={12} /> {t("misc.androidDevice.home")}
            </button>
            <button
              onClick={() => send({ type: "key", key: "recent" })}
              className="flex items-center justify-center gap-1.5 rounded-lg bg-raised py-2 text-[12px] text-ink hover:bg-raised-hover"
            >
              <RotateCcw size={13} /> {t("misc.androidDevice.recents")}
            </button>
          </div>
          <div className="text-center text-[11px] leading-relaxed text-ink-secondary">
            {t("misc.androidDevice.controlHint")}
          </div>
        </>
      )}
